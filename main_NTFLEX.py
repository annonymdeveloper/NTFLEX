...
from TFLEX.ComplexTemporalQueryDataloader import TestDataset, TrainDataset
from NTFLEX_DSL import is_to_predict_entity_set, is_to_predict_timestamp_set, query_contains_union_and_we_should_use_DNF
...
from TFLEX.assistance.toolbox.utils.KGArgsParser import KGEArgParser
from TFLEX.assistance.toolbox.utils.Log import Log
...
from ComplexNumericalQueryData import WIKI, WIKI_Forest_Fire_Sample, WIKI_Interval, WIKI_Random_Walk_Sample, ComplexNumericalQueryDatasetCachePath, NumericalComplexQueryData, TYPE_train_queries_answers, groups
from ParamSchemaExtension import is_entity, is_relation, is_timestamp, is_attribute, is_value

from dataclasses import dataclass, field
import os
import sys
from TFLEX.assistance.toolbox.utils.KGArgs import OutputArguments
from TFLEX.assistance.toolbox.utils.KGArgsParser import KGEArgParser
...
class NTFLEX(nn.Module):
    ...
    def build_neural_ops(self):
        ...
        def ValueNot(q):
            feature, logic, time_feature, time_logic, value_feature, value_logic = q
            return self.value_negation(feature, logic, time_feature, time_logic, value_feature, value_logic)
        
        def greaterPAx(e1, a1, t1):
            return ValueGreater2(AttributeValueProjection2(e1, a1, t1))

        def smallerPAx(e1, a1, t1):
            return ValueSmaller2(AttributeValueProjection2(e1, a1, t1))
    ...
    def build_parser(self):
        ...
        return TFLEX.expression.NeuralParser(neural_ops)
    ...
    def batch_predict(self, grouped_query: Dict[QueryStructure, torch.Tensor],
        ...
        for query_structure in grouped_query:
            ...
            if query_contains_union_and_we_should_use_DNF(query_name):
                ...
                elif is_to_predict_timestamp_set:
                    all_union_predict_1_t.append(predict_1)
                    all_union_predict_2_t.append(predict_2)
                    all_union_idxs_t.extend(query_idxs)
                else:
                    all_union_predict_1_v.append(predict_1)
                    all_union_predict_2_v.append(predict_2)
                    all_union_idxs_v.extend(query_idxs)
            else:
                # other query and DM are normal
                func = self.parser.fast_function(query_name)
    ...
    def forward_predict(
        ...
        if query_contains_union_and_we_should_use_DNF(query_name):
            ...
            if is_to_predict_entity_set(query_name):
                return self.scoring_to_answers(answer, all_union_predict, predict_entity=1, DNF_predict=True)
            elif is_to_predict_timestamp_set(query_name):
                return self.scoring_to_answers(answer, all_union_predict, predict_entity=2, DNF_predict=True)
            else:
                return self.scoring_to_answers(answer, all_union_predict, predict_entity=3, DNF_predict=True)
        else:
            # other query and DM are normal
            func = self.parser.fast_function(query_name)
            embedding_of_args = self.embed_args(query_args, query_tensor)  # (B, d)*L
    ...
    def scoring_to_answers(self, answer_ids: torch.Tensor, q: TYPE_token, predict_entity=True, DNF_predict=False):
        ...
        if predict_entity == 1:
            ...
            feature = self.value_feature(answer_ids).unsqueeze(dimn=1)
            scores = self.scoring_value(feature, q)
...
class MyExperiment(Experiment):
    ...
    def __init__(self, output: OutputSchema, data: NumericalComplexQueryData, model, args: TrainingArguments):
...
def build_data(args_data: DataArguments) -> NumericalComplexQueryData:
    if args_data.dataset == "WIKI":
        dataset = WIKI(args_data.data_home)
    elif args_data.dataset == "WIKI_forest_fire_samples":
        dataset = WIKI_Forest_Fire_Sample(args_data.data_home)
    elif args_data.dataset == "WIKI_random_walk_samples":
        dataset = WIKI_Random_Walk_Sample(args_data.data_home)
    elif args_data.dataset == "WIKI_interval":
        dataset = WIKI_Interval(args_data.data_home)
...
def build_model(args_model: ModelArguments, data: NumericalComplexQueryData):
    entity_count = data.entity_count
